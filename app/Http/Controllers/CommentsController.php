...
//        dd($request);
        ...
        if ($request->ajax()) {
            foreach ($coms as $com) {
                ...
                $data .= '"><div class="cm_like"></div><span id="like_count">'.$com->thisLikes->count().'</span></button><button class="btn cm_qsts"><div class="cm_qst"></div>Жауап жазу</button></div><form action="" id="ajax_form" method="post" class="cm_form a" style="display:none"><input type="text" class="form-control cm_input" name="text" placeholder="';
                Auth::user() ? $data.='Өз пікіріңізді жазыңыз...' : $data.='Пікірді тек тіркелген қолданушылар қалдыра алады...'; $data .= '" autocomplete="off"><input type="hidden" name="comment_id" class="comment_id" value="'.$com->id.'"><button type="button" class="btn-primary btn cm_btn">Жіберу <img src="'.asset("images/news/send.svg").'" alt=""></button></form>';
