<template>
    <head>
        <title>Админ панель | Тест нұсқа бойынша апеляция</title>
    </head>
    <AdminLayout>
        <template #breadcrumbs>
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Тест нұсқа бойынша тізімі</h1>
                </div>
                <div class="col-sm-6">
        ...
        <template #header>
            <div class="buttons">
                <Link
                    ...
                    :href="route('admin.test.optionQuestionAppeals.index')"
                >
                            ...
                            >
                                ...
                                <tbody>
                                    ...
                                    >
                                        ...
                                        <td >
                                            <b>{{ appeal.test.id }}</b>
                                            <templa v-if="appeal.test.subject">
                                                <br>
                                                 <b>Пән:</b> {{ appeal.test.subject.name }}
                                                <template v-if="appeal.test.option">
                                                    <br>
                                                    <b> Нұсқа:</b> {{ appeal.test.option.name }}
                                                </template>
                                            </templa>
                                        </td>
...
export default {
    ...
    methods: {
        deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(
                        route("admin.test.optionQuestionAppeals.destroy", id)
                    );
                }
            });
        },
        search() {
            ...
            this.$inertia.get(route("admin.test.optionQuestionAppeals.index"), params);
        },
    },
};
</script>
